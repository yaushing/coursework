...
try:
    ...
    raise ModuleNotFoundError(
        "Could not find required module {langchain}. Try re-running the install command."
    )
try:
    import pgzrun, pygame
except:
    raise ModuleNotFoundError(
        "Could not find required module {pygame}. Try re-running the install command."
    )
try:
    ...
    raise ModuleNotFoundError(
        "Could not find required module {matplotlib}. Try re-running the install command."
    )
try:
    import numpy as np
except:
    raise ModuleNotFoundError(
        "Could not find required module {numpy}. Try re-running the install command."
    )
try:
    ...
    raise ModuleNotFoundError(
        "Could not find built-in modules. Try re-installing python 3.9.12"
    )
try:
    import nltk
    ...
    warnings.warn(
        "Could not find built-in nltk sentiment analysis tools. App will run, but mood charts will not be generated."
    )
try:
    import password
    ...
    warnings.warn(
        "Configuration file for openAI API keys not found. Chatbot will be disabled"
    )
    chatbot_on = False
...
x_shift, y_shift = (
    0,
    0,
)  # Shifts rooms in relation to player (player stays in the centre of the screen)
robot_speaking = (
    False  # Makes game decide whether to display speech bubble for the robot
)
...
player_offset_x, player_offset_y = (
    0,
    0,
)  # Player offset to fit animations and movement along the x and y axis (0.25 offset, frame 1, 0.5 offset, frame 2... 1 offset = x += 1, frame 0 again)
...
OBJECTS = {
    ...
    2: [
        ...
        images.full_shadow,
        "a wall.",
        "Sterile, and devoid of contamination.",
    ],
    3: [
        [
            images.pillar,
    ...
    4: [
        images.soil,
        None,
        "soil, used for the farm.",
        "Surprisingly, it hasn't spilled onto the ground yet.",
    ],
    5: [
        [images.door, images.door1, images.door2, images.door3, images.door4],
        [
            images.door_shadow,
            images.door1_shadow,
            images.door3_shadow,
    ...
    12: [
        ...
        images.full_shadow,
        "a bookshelf.",
        "It's stacked with reference books.",
    ],
    13: [
    ...
    14: [
        ...
        images.half_shadow,
        "a small locker.",
        "It's used for storing personal items",
    ],
    15: [
        images.desk_computer,
        images.half_shadow,
        "a computer.",
        "It has logs from over for over 2000 years.",
    ],
    16: [
        images.plant,
        images.plant_shadow,
        "a spaceberry plant.",
        "It's locally sourced, sustainable and healthy!",
    ],
    17: [
        images.electrical1,
        images.half_shadow,
        "a part of the electrical system of the space shuttle.",
        "",
    ],
    18: [
        images.electrical2,
    ...
    20: [
        ...
        images.shrub_shadow,
        "a space lettuce.",
        "It's a bit limp, but amazing it's growing here!",
    ],
    21: [
        images.pipes1,
        images.pipes1_shadow,
        "a part of the water purification system of the space shuttle.",
        "",
    ],
    22: [
        images.pipes2,
        images.pipes2_shadow,
        "a part of the life support systems.",
        "Don't touch thease.",
    ],
    23: [
        images.pipes3,
    ...
    25: [
        ...
        "a scientific experiment of one of the old astronauts.",
    ...
    26: [
        ...
        "a robot arm.",
        "It was used for heavy lifting.",
    ],
    27: [images.toilet, images.half_shadow, "a toilet.", "It's sparkling clean."],
    28: [
        images.sink,
        None,
        "a sink, with clean running water recycled through the system.",
        "It's clean!",
    ],
    29: [
        images.globe,
    ...
    30: [
        ...
        "Martian soil and dust is on it.",
    ],
    31: [
        images.vending_machine,
        images.full_shadow,
        "a vending machine.",
        "Unfortunately, it needs a credit. And I used the last one.",
    ],
    33: [
        images.mission_control_desk,
        images.mission_control_desk_shadow,
        "computer station connected to Mission Control.",
        "",
    ],
    34: [
        images.duckwall,
        images.full_shadow,
        "a whiteboard.",
        "It used to be used for brainstorming and planning.",
    ],
    35: [
        images.window,
        images.full_shadow,
        "a window.",
        "It allows you to look out at space.",
    ],
    36: [
        images.window_short,
        images.full_shadow,
    ...
    37: [
        ...
        images.robot_shadow,
        "a cleaning robot.",
        "It's turned off right now to conserve power.",
    ],
    40: [
        images.drone,
        None,
        "a delivery drone",
        "They used to whizz through the corridors like nobody's business.",
    ],
    41: [
        images.computer,
        images.computer_shadow,
        "a computer workstation",
        "Used for managing space station systems.",
    ],
    42: [
        images.map,
        images.full_shadow,
        "a map charting the path of the ship.",
        "It's very in-depth.",
    ],
    43: [
        images.bottomfence,
        None,
        "a fence.",
        "It stops you from walking onto the crops",
    ],
...
ROOMS = [
    ...
    [
        ...
        False,
        "space.",
        "As a developer, please tell me how you got out so I can patch this.",
    ],
    [
        14,
        10,
    ...
    [
        ...
        False,
        "the garden.",
        "The plants grow here. Tomatoes grow surprisingly well!",
    ],
]
...
################
def create_room(
    room_number, width, height, left=False, right=False, up=False, down=False
):
    ### Validations
    assert (
        0 <= room_number <= ROOM_MAP_HEIGHT * ROOM_MAP_WIDTH
    ), f"Room number is invalid. Expected 0 ≤ room number ≤ {ROOM_MAP_HEIGHT * ROOM_MAP_WIDTH - 1}, got {room_number}"
    assert width % 2 == 0, "Width must be even"
    assert height % 2 == 0, "Height must be even"
    assert (
        0 <= width < ROOM_SIZE
    ), f"Width ({width}) must between 0 and ROOM_SIZE ({ROOM_SIZE}), inclusive."
    assert (
        0 <= height < ROOM_SIZE
    ), f"Height ({height}) must be between 0 and ROOM_SIZE ({ROOM_SIZE}), inclusive."
    if width == 0 or height == 0:
        return [
            [0 for _ in range(ROOM_SIZE)] for _ in range(ROOM_SIZE)
        ]  # if width or height of room is zero, return a void
    # Generates borders for empty void of space
    lr_borders = (ROOM_SIZE - width) // 2
    ud_borders = (ROOM_SIZE - height) // 2
    room = (
        [[2 for _ in range(width)]]
        + [[2] + [1 for _ in range(width - 2)] + [2] for _ in range(height - 2)]
        + [[2] + [3 for _ in range(width - 2)] + [2]]
    )
    if left:
        sections = height // 2
        room[sections - 1][0] = 1
    ...
    if room_number in SCENERY:
        ...
        for scenery_object in scenery_items_in_room:
            assert (
                0 <= scenery_object[1] + 1 <= len(room)
            ), f"Error in scenery placement: Expected -1 ≤ y ≤ {len(room) - 1}, got {scenery_object[1]}"
            assert (
                0 <= scenery_object[2] + 1 <= len(room[0])
            ), f"Error in scenery placement: Expected -1 ≤ x ≤ {len(room[0]) - 1}, got {scenery_object[2]}"
    ...
    room = (
        [[0 for _ in range(width)] for _ in range(ud_borders)]
        + room
        + [[0 for _ in range(width)] for _ in range(ud_borders)]
    )
    for i in range(ROOM_SIZE):
        room[i] = (
            [0 for _ in range(lr_borders)] + room[i] + [0 for _ in range(lr_borders)]
        )
    if left:
        sections = height // 2
...
def generate_rooms(rooms):
    room_data = []
    assert (
        len(rooms) == ROOM_MAP_HEIGHT * ROOM_MAP_WIDTH
    ), f"Expected {ROOM_MAP_HEIGHT * ROOM_MAP_WIDTH} rooms, got {len(rooms)}."
    for i in range(len(rooms)):
        if i % ROOM_MAP_WIDTH == 0:
            ROW_DATA = create_room(
                i,
                rooms[i][0],
                rooms[i][1],
                rooms[i][2],
    ...
    for y in range(len(room_data)):
        for x in range(len(room_data[y])):
            if (
                room_data[y][x] == 2
                and room_data[y + 1][x] == 0
                and room_data[y - 1][x] == 1
            ):
                room_data[y][x] = 3
    ...
    for row in room_data:
        if len(row) > 0:
            final.append(row)
    assert (
        len(final) == ROOM_MAP_HEIGHT * ROOM_SIZE
    ), f"Expected height to be {ROOM_MAP_HEIGHT * ROOM_SIZE}, got {len(room_data)}"
    for i in range(len(final)):
        assert (
            len(final[i]) == ROOM_MAP_WIDTH * ROOM_SIZE
        ), f"Expected width of row {i} to be {ROOM_MAP_WIDTH * ROOM_SIZE}, got {len(final[i])}"
        trans_wall_count += final[i].count(3)
    assert (
        trans_wall_count == 50
    ), f"Expected 50 transparent walls, got {trans_wall_count}."
    return final
...
def adjust_wall_transparency():
    ...
    # Basically a key
    checked_tiles = {
        (player_x - 1) * 2 * (player_y)
        - (player_x - 1) ** 2: room_map[player_y][player_x - 1],
        (player_x - 1) * 2 * (player_y + 1)
        - (player_x - 1) ** 2: room_map[player_y + 1][player_x - 1],
        (player_x - 1) * 2 * (player_y + 2)
        - (player_x - 1) ** 2: room_map[player_y + 2][player_x - 1],
        (player_x) * 2 * (player_y) - (player_x) ** 2: room_map[player_y][player_x],
        (player_x) * 2 * (player_y + 1)
        - (player_x) ** 2: room_map[player_y + 1][player_x],
        (player_x) * 2 * (player_y + 2)
        - (player_x) ** 2: room_map[player_y + 2][player_x],
        (player_x + 1) * 2 * (player_y)
        - (player_x + 1) ** 2: room_map[player_y][player_x + 1],
        (player_x + 1) * 2 * (player_y + 1)
        - (player_x + 1) ** 2: room_map[player_y + 1][player_x + 1],
...
def draw_image(image, y, x):
    screen.blit(
        image,
        (
            top_left_x + ((x + x_shift) * TILE_SIZE),
            top_left_y + ((y + y_shift) * TILE_SIZE) - image.get_height(),
        ),
    )
...
def draw_shadow(image, y, x):
    screen.blit(
        ...
        (
            top_left_x + ((x + x_shift) * TILE_SIZE),
            top_left_y + ((y + y_shift) * TILE_SIZE),
        ),
    )
...
def draw_player():
    ...
    screen.blit(
        player_image,
        (
            top_left_x + 15 * TILE_SIZE,
            top_left_y + 15 * TILE_SIZE - player_image.get_height(),
        ),
    )
    player_image_shadow = PLAYER_SHADOW[player_direction][player_frame]
    screen.blit(
        player_image_shadow, (top_left_x + 15 * TILE_SIZE, top_left_y + 15 * TILE_SIZE)
    )
...
def draw_robot():
    ...
    screen.blit(
        robot_image,
        (
            top_left_x + (robot_x + x_shift + robot_offset_x) * TILE_SIZE,
            top_left_y
            + (robot_y + y_shift + robot_offset_y) * TILE_SIZE
            - robot_image.get_height(),
        ),
    )
    ...
    if player_speaking:  # Typing
        if len(key_id) == 1:
            if mod:
                ...
                player_text += key_id.lower()
        if len(key_id) == 3:
            if mod:
                player_text += SHIFTED[int(key_id[-1])]
            else:
                player_text += key_id[-1]
        if key_id == "BACKQUOTE":
            if mod:
                player_text += "~"
            else:
...
def display_robot_message_cont():
    ...
    split_text = [
        future_robot_text[i : i + 200] for i in range(0, len(future_robot_text), 200)
    ]
...
def draw():
    ...
    screen.blit(
        images.backdrop,
        (
            top_left_x + ((-15 + x_shift) * TILE_SIZE),
            top_left_y + ((-15 + y_shift) * TILE_SIZE),
        ),
    )
    ...
    for y in range(ROOM_MAP_HEIGHT * ROOM_SIZE):
        for x in range(ROOM_MAP_WIDTH * ROOM_SIZE):
            ...
            if item_here not in ITEMS_PLAYER_MAY_STAND_ON + [255] or item_here in [
                5,
                43,
                45,
                46,
    ...
    if exercise_menu and not paused:
        s = pygame.Surface(
            (WIDTH - 70, HEIGHT - 70), pygame.SRCALPHA
        )  # Creates a surface
        ...
        if 120 <= mouse_x <= 780 and 280 <= mouse_y <= 420:
            screen.blit(images.breathing_button_hover, (120, 280))
        else:
            screen.blit(images.breathing_button, (120, 280))

        if 120 <= mouse_x <= 780 and 460 <= mouse_y <= 600:
            screen.blit(images.bodyscan_button_hover, (120, 460))
        else:
            screen.blit(images.bodyscan_button, (120, 460))

    if breathing_menu and not paused:
        s = pygame.Surface(
            (WIDTH - 70, HEIGHT - 70), pygame.SRCALPHA
        )  # Creates a surface over exercise menu
        ...
        screen.draw.text(
            "press key [RIGHT] to go to the next stage.",
            ((WIDTH / 2 - 485 / 2), 200),
            color="dimgrey",
            width=485,
            fontname="biorhyme",
        ...
        if cur_pg < 1:
            screen.draw.text(
                "Deep breathing helps to relieve symptoms of anxiety. ",
                ((WIDTH / 2 - 485 / 2), 270),
                color="black",
                width=485,
                fontname="biorhyme",
            ...
            screen.draw.text(
                "INSTRUCTIONS: Lean back or lie down in a comfortable position. Close your eyes if you’d like. When starting out, try placing a hand on your stomach. If you breathe deeply enough, you should feel it rise and fall with each breath. ",
                ((WIDTH / 2 - 485 / 2), 340),
                color="black",
                width=485,
                fontname="biorhyme",
        ...
        # back button hover
        if 90 <= mouse_x <= 200 and 80 <= mouse_y <= 115:
            screen.blit(images.exercise_back_hover, (90, 80))
        else:
            screen.blit(images.exercise_back, (90, 80))
        ...
        if cur_pg >= 2:
            ...
            screen.draw.text(
                "Pause. Hold your breath for 4 seconds.",
                (170, 340),
                color="dimgrey",
                width=550,
                fontname="biorhyme",
        ...
        if cur_pg >= 3:
            ...
            screen.draw.text(
                "Exhale. Gently release your breath through your mouth for 6 seconds.",
                (170, 388),
                color="dimgrey",
                width=550,
                fontname="biorhyme",
            ...
            screen.draw.text(
                "Tip: Purse your lips as if blowing through a straw to slow the exhale.",
                (170, 446),
                color="dimgrey",
                width=550,
                fontname="biorhyme",
        ...
        if cur_pg >= 4:
            ...
            screen.draw.text(
                "Repeat. Continue for at least 2 minutes, ideally 5 to 10 minutes.",
                (170, 535),
                color="dimgrey",
                width=550,
                fontname="biorhyme",
            ...
            screen.draw.text(
                "You'll likely feel calmer after this exercise, but if not, don't worry—it takes practice.",
                ((WIDTH / 2 - 485 / 2), 630),
                color="black",
                width=485,
                fontname="biorhyme",
    ...
    if bodyscan_menu and not paused:
        ...
        screen.draw.text(
            "press key [SPACE] to start, pause and resume the audio track.",
            ((WIDTH / 2 - 485 / 2), 200),
            color="dimgrey",
            width=485,
            fontname="biorhyme",
        ...
        screen.draw.text(
            "For this exercise, you will focus on the physical sensations in your body. Simply observe these sensations through listening to the audio track.",
            ((WIDTH / 2 - 485 / 2), 275),
            color="black",
            width=485,
            fontname="biorhyme",
    ...
    if paused:
        s = pygame.Surface(
            (WIDTH, HEIGHT)
        )  # Creates a surface the height and width of the window
        ...
        if 270 <= mouse_x <= 630 and 250 <= mouse_y <= 340 and not modal:
            screen.blit(images.maincontinuehover, (270, 250))
        else:
            screen.blit(images.maincontinue, (270, 250))
        if 270 <= mouse_x <= 630 and 390 <= mouse_y <= 480 and not modal:
            screen.blit(images.mainquithover, (270, 390))
        else:
            screen.blit(images.mainquit, (270, 390))
        if not mute:
            if 270 <= mouse_x <= 360 and 530 <= mouse_y <= 620 and not modal:
                screen.blit(images.mutehover, (270, 530))
            else:
                screen.blit(images.mute, (270, 530))
        else:
            if 270 <= mouse_x <= 360 and 530 <= mouse_y <= 620 and not modal:
                screen.blit(images.unmutehover, (270, 530))
            else:
                screen.blit(images.unmute, (270, 530))
        if 540 <= mouse_x <= 630 and 530 <= mouse_y <= 620 and not modal:
            screen.blit(images.deldatahover, (540, 530))
        else:
            screen.blit(images.deldata, (540, 530))
    if modal:
        ...
        if 285 <= mouse_x <= 465 and 520 <= mouse_y <= 570:
            screen.blit(images.modalcontinuehover, (285, 520))
        else:
            screen.blit(images.modalcontinue, (285, 520))
        if 465 <= mouse_x <= 615 and 520 <= mouse_y <= 570:
            screen.blit(images.modalcancelhover, (465, 520))
        else:
            screen.blit(images.modalcancel, (465, 520))
    if displaying_chart:
...
def game_loop():
    ...
    if player_frame == 0:
        ...
        elif not exercise_menu and (
            keyboard.right or keyboard.d
        ):  # elif stops player making diagonal movements
            from_player_x = player_x
            from_player_y = player_y
            player_x += 1
    ...
    if robot_direction == "down" and robot_moving > 0:
        robot_offset_y = -1 + (0.25 * player_frame)
    x_shift, y_shift = -(player_x + player_offset_x - 15), -(
        player_y + player_offset_y - 15
    )
